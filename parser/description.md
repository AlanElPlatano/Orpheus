...
* **Library recommendation:** primary: **miditoolkit** (works natively in MIDI ticks and builds on `mido`). Use `mido` as the low-level read/write backend when you need ultimate control; use `pretty_midi` only for quick analysis helpers (tempo estimation, chroma) when needed.
...
* **Other important considerations:** quantization strategy, token vocabulary (include velocity & ticks), chord-detection rules, sustain pedal & control changes, drums & channels, scaling/augmentation, performance & memory, deterministic sorting rules, and legal/copyright concerns.
...
**Format:** one JSON file per MIDI, with a schema that stores metadata and token arrays per track (or a single combined token stream if you prefer).

**Filename convention (sanitized):**
...
* Native support in Python and most ML tooling.
* Can store structured metadata and arrays (tokens) in a compact, compatible way.
* Easy to compress (gzip) and stream (NDJSON) for large corpora.
  ...
  "tracks": [
    {
      ...
      "is_drum": false,
      "type": "melody",
      "tokens": ["BAR_0","POS_0","NOTE_ON_60_80","WAIT_48","NOTE_OFF_60"]
    }
  ],
...
* Use integer tick units instead of floats to avoid rounding errors. `WAIT_{ticks}` is the recommended form.
* Include velocity in `NOTE_ON_{pitch}_{vel}` (important for human expressivity and to reconstruct velocities).
...
* For chords use `CHORD_{p1}_{p2}_{p3}_{vel1}_{vel2}_{vel3}` or just `CHORD_{p1}_{p2}_{p3}` plus velocities elsewhere — prefer the more unambiguous approach that contains velocities if you care about dynamics.

**File variants:**

* `.tok.json` — single JSON per piece (easiest to use).
* `.tok.json.gz` — compressed version for disk space.
* `.tok.jsonl` (NDJSON) — if you generate very large corpora and want streaming line-by-line loading.
...
TOKENS     ::= TOKEN+
TOKEN      ::= BAR_{bar_index} | POS_{pos_index} | WAIT_{ticks} | TEMPO_{bpm} |
              | NOTE_ON_{pitch}_{vel} | NOTE_OFF_{pitch} | CHORD_{p1}_{p2}_... | PROG_{program}
```
...
2. Control changes (e.g., sustain pedal) next.
3. NOTE\_OFF messages next.
4. NOTE\_ON messages last.
...
* You may quantize to a grid (e.g., 1/24 or 1/96 of a quarter) if you want a fixed resolution across files, but store the quantization base in metadata and use integer ticks for tokens.
...
* Group notes that start on the exact same tick (after quantization) and have at least `chord_threshold` notes (default 3).
...
4. **Record program changes and control change events** if they are musically relevant (sustain pedal, expression, volume) or if you want faithful playback.
5. **Define and document deterministic ordering** for simultaneous events (see above).
6. **Round-trip validation**: implement tests that compare the original MIDI and the round-tripped MIDI (tolerance for allowed differences, e.g., removed tiny timing noise if quantized). Store a small suite of metrics after conversion: total notes, number of unmatched note\_on/off events, percent of notes with changed start tick, mean start offset, etc.
...
1. **Note-only tokens (pitch + durations via WAIT / NOTE\_OFF):** small vocabulary, simpler, but loses velocity and expressive info.
...
* [ ] Decide how to handle control changes and pedals.
...
**pretty\_midi** — pros: high-level music-related analysis utilities (get\_tempo\_changes, chroma, estimate\_tempo); cons: often works in seconds and hides tick detail (be careful if you need exact tick-level round-trips).

**MidiTok / REMI / Compound Word** — these are *tokenization strategies* rather than pure I/O libraries; excellent references if you plan to train a Transformer or want to leverage existing tokenizers.
...
*If you want, I can now: produce example token-to-MIDI and MIDI-to-token test vectors (not code — but example JSON inputs/outputs), or draft the `config.py` defaults and the precise token vocabulary list you'd store in metadata.*
